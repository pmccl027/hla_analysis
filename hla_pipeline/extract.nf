...
process Extract_HLA {
	...
	samtools view -T ${params.reference} -f 4 -F 264 ${cram} -b -o all.1.bam

	# mapped reads w/ unmapped mate
	samtools view -T ${params.reference} -f 8 -F 260 ${cram} -b -o all.2.bam

	# both unmapped
	samtools view -T ${params.reference} -f 12 -F 256 ${cram} -b -o all.3.bam

	# mapped in HLA
	samtools view -T ${params.reference} -F 12 ${cram} chr6:25000000-35000000 -b -o hla.mapped_only.bam
