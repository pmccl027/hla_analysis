process Extract_HLA {
	...
	input:
        /// # infolder should incl /*.cram or /*.bam in config
	tuple file (bam), file(bam_index) from Channel.fromPath("${params.infolder}").map{ bam -> [ bam, bam + (bam.getExtension() == "bam" ? ".bai" : ".crai") ] }
